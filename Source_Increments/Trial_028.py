...
class ControlFlowGraph:
    ...
    def add_edge(self, u, v):
        ...
        # Validate node names (must be alphanumeric, underscores, or hyphens, no control characters)
        if re.search(r'[\x00-\x1F\x7F]', u+v) or any(c in u+v for c in map(chr, range(0x80, 0xA0))):
            raise ValueError(f"Invalid node name: '{u}' or '{v}' (contains control characters)")
        if not re.match(r'^[\w-]+$', u) or not re.match(r'^[\w-]+$', v):
            raise ValueError(f"Invalid node name: '{u}' or '{v}' (contains spaces or special characters)")
    ...
    def from_file(filename):
        ...
        if start not in cfg.nodes:
            print(f"Error: Start node '{start}' is not part of any edge.")
            sys.exit(1)
        if end not in cfg.nodes:
            print(f"Error: End node '{end}' is not part of any edge.")
            sys.exit(1)
