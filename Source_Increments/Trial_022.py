...
class ControlFlowGraph:
    ...
    def _dfs(self, node, end, visited_nodes, path):
        ...
        if node == end:
            ...
            visited_nodes.add(node)  # Mark node as visited for this path
            for neighbor in self.graph[node]:
                if neighbor not in path or neighbor == end:
                    self._dfs(neighbor, end, visited_nodes, path)
            visited_nodes.remove(node)  # Backtrack
    ...
    def from_file(filename):
        ...
        with open(filename, 'r') as file:
            ...
            for line in lines[1:-1]:  # Process the edges in between
                u, v = map(str.strip, line.split())  # Strip whitespace
                cfg.add_edge(u, v)  # Add edge to graph
        
        # Debugging output to verify nodes and edges
        print("Nodes in Graph:", cfg.nodes)
        print("Edges in Graph:", cfg.edges)
