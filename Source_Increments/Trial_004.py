...
class ControlFlowGraph:
        ...
        if node == end:
            ...
            for neighbor in self.graph[node]:
                if neighbor not in visited or neighbor == end:
                    # Explore unvisited neighbors or revisit the end node
                    self._dfs(neighbor, end, visited, path)
            visited.remove(node)  # Backtrack to explore other paths
    ...
    def _extract_basis_set(self):
        ...
        for path in self.paths:
            ...
            if len(basis_set) == cyclomatic_complexity:
                break
...
if __name__ == "__main__":
    ...
    # Print the number of basis paths
    print(f"Number of Basis Paths: {cfg.basis_path_count}")
