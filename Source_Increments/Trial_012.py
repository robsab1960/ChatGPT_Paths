...
class ControlFlowGraph:
    ...
    def find_basis_set(self, start, end):
        ...
        path = []
        self.paths.clear()
        self._dfs(start, end, set(), path, visited_edges)
        return self._extract_basis_set()

    def _dfs(self, node, end, visited_nodes, path, visited_edges):
        # Detect cycles: If a node is revisited in the same path, stop recursion
        if node in visited_nodes:
            return
        
        path.append(node)  # Add current node to path
        visited_nodes.add(node)  # Mark the node as visited in the current path
        ...
        visited_nodes.remove(node)  # Remove the node from visited set on backtrack
