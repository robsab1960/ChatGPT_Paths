...
class ControlFlowGraph:
    ...
    def from_file(filename):
        ...
        # Check that no Edges Start in end
        # Check that no Edges End in start
        
        for line in lines[1:-1]:
            parts = line.split()
            u, v = map(str.strip, parts)
            if u == end:
                print(f"Error: end node should not start an edge")
                sys.exit(1)
            if v == start:
                print(f"Error: start node should not end an edge")
                sys.exit(1)
