...
class ControlFlowGraph:
    ...
    def find_basis_set(self, start, end):
        ...
        path = []
        self.paths.clear()
        self._dfs(start, end, visited_nodes, path, set())
    ...
    def _dfs(self, node, end, visited_nodes, path, visited_edges):
        ...
        if node == end:
            ...
            for neighbor in self.graph[node]:
                edge = (node, neighbor)
                if edge not in visited_edges:  # Ensure all edges are explored
                    visited_edges.add(edge)
                    self._dfs(neighbor, end, visited_nodes, path, visited_edges)
                    visited_edges.remove(edge)
            visited_nodes.discard(node)  # Backtrack safely
    ...
    def _extract_basis_set(self):
        ...
        for path in self.paths:
            ...
            # Continue until we have enough independent paths
            if len(basis_set) == expected_basis_size:
                break
        
        # Ensure we have exactly the expected number of basis paths
        while len(basis_set) < expected_basis_size:
            for path in self.paths:
                if path not in basis_set:
                    basis_set.append(path)
                if len(basis_set) == expected_basis_size:
                    break
