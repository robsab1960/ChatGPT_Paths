...
def artifacts_dir(request) -> Path:
    """
    Per-test, per-execution artifact directory.

    Layout:
      Test_Artifacts/<timestamp>_<pid>/<sanitized nodeid>/
    """
    ...
    test_dir = run_dir / _sanitize_nodeid(request.node.nodeid)
...
def pytest_collection_modifyitems(config, items):
    ...
    And (optional) warn if idea-source markers don't match the objective's ideas list.
    """
    missing = []
    multi = []
    mismatches = []
...
def pytest_collection_finish(session):
    """
    Write an objectives index file every run (handy for traceability, reviews, and mapping coverage).
    """
    objs = []
    for item in session.items:
        ...
        objs.append({
            ...
            "idea_tags_present": [name for name in IDEA_MARKERS.values() if item.get_closest_marker(name)],
        })
...
class ChecksRecorder:
    ...
    def check(self, condition: bool, label: str, details: Optional[dict[str, Any]] = None, hard: bool = True) -> bool:
        """
        Records PASS/FAIL. If hard=True and condition is False, fails the test via assert.
        Returns the condition so you can use it inline if you want.
        """
        entry = {"label": label, "pass": bool(condition)}
        if details:
        ...
        if details:
            # one-line JSON details (easy to scan, still parseable)
    ...
    def note(self, label: str, details: Optional[dict[str, Any]] = None) -> None:
        """Records a non-check observation line (always continues)."""
        entry = {"label": label, "note": True}
        if details:
            entry["details"] = details
        self.entries.append(entry)
...
@pytest.fixture
def checks(request, artifacts_dir) -> ChecksRecorder:
    """
    Per-test recorder that writes checks.log and also emits objective metadata into the artifacts folder.
    """
    rec = ChecksRecorder(artifacts_dir=artifacts_dir)

    # Pull objective marker metadata (enforced to exist by your collection hook)
    ...
    # Write an objective.json artifact
    (artifacts_dir / "objective.json").write_text(json.dumps(obj, indent=2), encoding="utf-8")

    # Also prepend a human-readable header into checks.log
