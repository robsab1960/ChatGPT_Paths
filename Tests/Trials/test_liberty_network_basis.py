...
LIBERTY_INPUT = TESTDATA / "LibertyInput.txt"
PATHS_JAR = TOOLS / "paths.jar"


def run_paths_jar(liberty_input: Path) -> tuple[int, str, str]:
    cmd = ["java", "-jar", str(PATHS_JAR), str(liberty_input), "-basis"]
...
def run_path_master_json(tool_path: Path, liberty_input: Path, extra_args=None) -> tuple[int, dict, str, str]:
    extra_args = extra_args or []
    cmd = ["python", str(tool_path), "--json", "--confirm-basis", *extra_args, str(liberty_input)]
    p = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
...
@pytest.mark.objective(
    id="OBJ-LIB-001",
    text="Confirm Path_Master.py can generate a set of basis paths for transactions to flow through the Liberty Network. For comparison purposes a baseline set of basis paths is generated with the java program paths.jar.",
    ideas=["Capabilities", "Examples"],
)
def test_liberty_network_basis_enum_and_linear(tool_path, artifacts_dir, checks):
    # Preconditions
    checks.check(LIBERTY_INPUT.exists(), f"Precondition: Liberty input exists: {LIBERTY_INPUT}")
    ...
    rc_j, out_j, err_j = run_paths_jar(LIBERTY_INPUT)
    ...
    rc_e, pm_e_json, pm_e_stdout, pm_e_stderr = run_path_master_json(Path(tool_path), LIBERTY_INPUT, extra_args=[])
    ...
    # --- Path_Master linear ---
    rc_l, pm_l_json, pm_l_stdout, pm_l_stderr = run_path_master_json(Path(tool_path), LIBERTY_INPUT, extra_args=["--linear"])
